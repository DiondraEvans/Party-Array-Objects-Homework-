...
const alteringhpandbelongings = (name, hp) =>{
    party.forEach(element => {
        if(element.name == name){
            element.hitpoints = element.hitpoints + hp;
...
alteringhpandbelongings('Timothy', 20);
console.log(party[1]);
// Prompt 5: Timothy got hungry and stole Joline's bread. Take it out of her belongings and put it  into Timothy's belongings.
const assignedbelongings = (name, personstolefrom) => {
    party.forEach(element => {
      if (element.name == personstolefrom) {
        stolenitem = element.belongings.splice(1, 1);
      }
      else if (element.name == name) {
        element.belongings.push(...stolenitem);
      }
    });
  }
  assignedbelongings("Timothy", "Joline");
  console.log(party);
